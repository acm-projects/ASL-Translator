...
Future<String> getImageLabel(String imagePath) async{
  ...
  // final fixedFile = await originalFile.writeAsBytes(fixedImage);

  // Image

  // final bytes = await Io.fixedImage.readAsBytes();
  // final bytes = await originalFile.writeAsBytes(img.encodeJpg(fixedImage));
  // final bytes2 = await fixedImage.readAsBytes();

  // final List<int> bytes = await Io.File(imagePath).readAsBytes(); //return to this if hell breaks loose
